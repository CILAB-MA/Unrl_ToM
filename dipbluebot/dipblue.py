...
class DipBlue:
    ...
    def __init__(self, static_dict, me, weights):
        ...
        for power_name in self.powers:
            ...
            self.ratio[power_name] = 1.0
            self.peace[power_name] = False
            self.ally[power_name] = False
            self.war[power_name] = False
            self.was_war[power_name] = False
    ...
    def _update_nego_variables(self, eliminated, loc2power, power2loc, centers):
        self.negotiator.peace = self.peace
        for power in self.powers:
            self.negotiator.effective_ratio[power] = self.get_effective_ratio(power)
    ...
    def _nego_act(self, obs, controlled_regions):
        ...
        if self.step == 0:
            ...
            self.peace = self.negotiator.peace
            # update the place where need support
            self.dipblue_handler.evaluated_move(controlled_regions, obs, clear_agreed=False)
            self.other_need_support = self.dipblue_handler.other_need_support
            self.negotiator.other_need_support = self.other_need_support
            ...
            for msg in recv_msgs:
                message = self.negotiator.handle_negotation_message(msg[1], msg[0]['message'],
                                                                    msg[0]['sender'], obs['units'],
                                                                    obs['name'][-2])
                #print(self.negotiator.agreed_orders)
                self.peace = self.negotiator.peace
                self.war = self.negotiator.war
                self.agreed_orders = self.negotiator.agreed_orders
                self.ratio = self.negotiator.ratio
    ...
    def _order_act(self, obs, controlled_regions):
        ...
        if obs['name'][-1] == 'M':
            recv_msgs = obs['messages'][self.me]
            for msg in recv_msgs:
                self.negotiator.handle_negotation_message(msg[1], msg[0]['message'],
                                                                    msg[0]['sender'], obs['units'],
                                                                    obs['name'][-2])
            ...
            self._update_order_variables()
            orders, agreed_orders = self.dipblue_handler.evaluated_move(controlled_regions, obs, clear_agreed=True)
        ...
        if (obs['prev_orders'][self.me] != None) and len(sum(self.promised_orders.values(), [])) > 0:
            if len(obs['prev_orders'][self.me]) > 0:
                self.ratio = self.dipblue_handler.action_ratio(obs, self.powers, self.ratio)
                self.ratio = self.dipblue_handler.agreed_order_ratio(obs['prev_orders'], self.ratio)
