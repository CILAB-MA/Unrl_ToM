...
def run_one_loader(train_loader, tom_net, criterion, optimizer, device, no_char=False):
    ...
    for i, batch in enumerate(train_loader):
        c_board, c_src, c_message, t_order, t_dst, t_response, \
            me_weights, other_weights, me_ind, other_ind, c_send, c_order, c_other_internal, t_true_prob = batch
        ...
        c_other_internal = c_other_internal.float().to(device)
        ...
        me_weights = me_weights.to(device)
        other_weights = other_weights.to(device)
...
def run_experiment(exp_dir, configs):
    ...
    '''setting env & models'''
    oracle_net = FC_PredNet(device=device, norm_adjacency=norm_adjacency, configs=configs)
    oracle_net.to(device)
    ...
    optimizer = tr.optim.Adam(oracle_net.parameters(), lr=configs['lr'])
    ...
    for epoch in range(configs['num_epoch']):
        ...
        for num_dataset in dataset_num:
            ...
            results, i, visuals, optimizer = run_one_loader(train_loader, oracle_net, criterion, optimizer, device, configs['no_char'])
        ...
        for i, batch in enumerate(test_loader):
            with tr.no_grad():
                ...
                me_ind = me_ind.to(device)
                other_ind = other_ind.to(device)
                pred_order, pred_dst, pred_response, e_char = oracle_net(c_board, c_src, c_message, c_send,  c_order,
        ...
        if epoch % configs['save_freq'] == 0 and configs['save_model'] and epoch >= configs['save_start_epoch']:
            if test_epoch_loss < best_loss:
                ...
                save_model(oracle_net, exp_dir, model_info)
                early_stopping = 0
            else:
                early_stopping += 1
