...
def run_one_loader(train_loader, tom_net, criterion, optimizer, device, no_char=False):
    ...
    for i, batch in enumerate(train_loader):
        ...
        pred_order, pred_dst, pred_response, e_char, raw_e_char = tom_net(p_board, p_order, p_message, c_board,
                                                                          c_src, c_message,
...
def run_experiment(exp_dir, configs):
    ...
    if configs['map_type'] == 'standard':
        ...
        norm_adjacency = None
    norm_adjacency = torch.tensor(norm_adjacency).to(device)
    ...
    for epoch in range(configs['num_epoch']):
        ...
        for i, batch in enumerate(test_loader):
            with tr.no_grad():
                ...
                me_ind = me_ind.float().to(device)
                t_true_prob = t_true_prob.to(device)
                pred_order, pred_dst, pred_response, e_char, e_char_raw = tom_net(p_board, p_order, p_message, c_board,
                                                                                  c_src, c_message,
                                                                                  c_send, c_order, other_ind, me_ind)
                order_loss = criterion(pred_order, t_order)
                dst_loss = criterion(pred_dst, t_dst)
