...
def run_one_loader(train_loader, tom_net, criterion, optimizer, device, no_char=False):
    ...
    for i, batch in enumerate(train_loader):
        ...
        pred_order, pred_dst, pred_response, e_char = tom_net(p_board, p_order, p_message, c_board, c_src, c_message,
                                                              c_send, c_order, other_ind, me_ind)
...
def run_experiment(exp_dir, configs):
    ...
    for epoch in range(configs['num_epoch']):
        ...
        #     other_powers_list = np.concatenate(total_train_other_powers_list, axis=0)
        ...
        #     # pil_3d_image_me = visualizer.get_char(e_chars_list, epoch, me_weights_list)
        #     # pil_3d_image_other = visualizer.get_char(e_chars_list, epoch, other_weights_list)
        #     pil_image_power_other = visualizer.get_power_char(e_chars_list, epoch, other_powers_list, other_weights_list)
        #
        #
        ...
        #         # wandb.log({'Train 3d Image(me_weight)': wandb.Image(pil_3d_image_me)}, step=epoch)
        #         # wandb.log({'Train 3d Image(other_weight)': wandb.Image(pil_3d_image_other)}, step=epoch)
        #         wandb.log({'Test power Image(other_weight)': wandb.Image(pil_image_power_other)}, step=epoch)
        ...
        for i, batch in enumerate(test_loader):
            with tr.no_grad():
                ...
                pred_order, pred_dst, pred_response, e_char = tom_net(p_board, p_order, p_message, c_board, c_src,
                                                                      c_message,
                                                                      c_send, c_order, other_ind, me_ind)
                order_loss = criterion(pred_order, t_order)
                dst_loss = criterion(pred_dst, t_dst)
                ...
                if not configs['no_char']:
                    ...
                    test_other_powers_list.append(other_ind[:,-1].cpu().detach().squeeze(-1).numpy())
        ...
        #     test_other_powers_list = np.concatenate(test_other_powers_list, axis=0)
        ...
        #     # pil_3d_image_me = visualizer.get_3d_char(test_e_chars_list, epoch, test_me_weights_list)
        #     # pil_3d_image_other = visualizer.get_3d_char(test_e_chars_list, epoch, test_other_weights_list)
        #     pil_image_power_other = visualizer.get_power_char(test_e_chars_list, epoch, test_other_powers_list, test_other_weights_list)
        #
        #     if configs['use_wandb']:
        ...
        #         # wandb.log({'Test 3d Image(me_weight)': wandb.Image(pil_3d_image_me)}, step=epoch)
        #         # wandb.log({'Test 3d Image(other_weight)': wandb.Image(pil_3d_image_other)}, step=epoch)
        #         wandb.log({'Test power Image(other_weight)': wandb.Image(pil_image_power_other)}, step=epoch)
