...
def run_one_loader(train_loader, lstm_net, criterion, optimizer, device, num_agent):
    ...
    for i, batch in enumerate(train_loader):
        ...
        me_ind = me_ind.to(device)
        other_ind = other_ind.to(device)

        pred_order, pred_dst, pred_response, e_char = lstm_net(c_board, c_src, c_message, c_send, c_order, me_ind,
                                                               other_ind)
...
def run_experiment(exp_dir, configs):
    ...
    '''setting env & models'''
    lstm_net = Only_lstm_PredNet(device=device, configs=configs)
    lstm_net.to(device)
    ...
    optimizer = tr.optim.Adam(lstm_net.parameters(), lr=configs['lr'])
    ...
    for epoch in range(configs['num_epoch']):
        ...
        for num_dataset in dataset_num:
            ...
            results, i, visuals, optimizer = run_one_loader(train_loader, lstm_net, criterion, optimizer, device, num_agent)
        ...
        if configs['use_wandb']:
            wandb.log({'Train/loss': total_train_epoch_loss, 'Train/order_acc': total_train_epoch_order_acc,
                       ...
                       'Train/true_response_prob_loss': total_train_epoch_true_response_prob_loss},
                      step=epoch)
        ...
        for i, batch in enumerate(test_loader):
            with tr.no_grad():
                ...
                other_ind = other_ind.to(device)
                t_true_prob = t_true_prob.to(device)
                pred_order, pred_dst, pred_response, e_char = lstm_net(c_board, c_src, c_message, c_send, c_order,
                                                                      me_ind, other_ind)
        ...
        print("Test Epoch: {} | true_order_loss: {:3f} | true_response_loss: {:3f} ".format(epoch, test_epoch_true_order_prob_loss, test_epoch_true_response_prob_loss))
        if configs['use_wandb']:
        ...
        if epoch % configs['save_freq'] == 0 and configs['save_model'] and epoch >= configs['save_start_epoch']:
            if test_epoch_loss < best_loss:
                ...
                save_model(lstm_net, exp_dir, model_info)
                early_stopping = 0
            else:
                early_stopping += 1
