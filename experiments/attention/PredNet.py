...
class FC_PredNet(nn.Module):
    def __init__(self, device, norm_adjacency, configs):
        ...
        if configs['no_char']:
            self.hidden_size = (configs['char_output_dim'] + configs['num_msg'] * configs['msg_feat']) + \
                               (configs['num_loc'] * configs['board_feat'])
        else:
    ...
    def forward(self, past_board, past_order, past_msg, curr_board, curr_src, curr_msg, curr_send, curr_order, other_ind, me_ind):
        ...
        if s == 0:
            ...
            e_char_2d, weights, e_char_total = self.e_char(past_board, past_order, past_msg, past_other_ind, past_me_ind)
        ...
        if self.no_char:
            ...
            return order, dst, response, e_char_2d, weights, e_char_total
