...
def run_one_loader(train_loader, tom_net, criterion, optimizer, no_char=False):
    ...
    for i, batch in enumerate(train_loader):
        ...
        pred_order, pred_dst, pred_response, e_char, weights = tom_net(p_board, p_order, p_message, c_board, c_src, c_message,
    ...
    for epoch in range(configs['num_epoch']):
        ...
        for i, batch in enumerate(test_loader):
            with tr.no_grad():
                ...
                me_ind = me_ind.float().cuda()
                other_ind = other_ind.float().cuda()
                pred_order, pred_dst, pred_response, e_char, weights = tom_net(p_board, p_order, p_message, c_board,
                                                                               c_src, c_message,
                                                                               c_send, c_order, other_ind, me_ind)
        ...
        if epoch % configs['save_freq'] == 0 and configs['save_model'] and epoch >= configs['save_start_epoch']:
            if test_epoch_loss < best_loss:
                ...
                early_stop += 1
                if early_stop >= configs["early_stop_num"]:
                    wandb.finish()
                    break
